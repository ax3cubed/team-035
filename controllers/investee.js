...
const investeeCreate = (req, res) => {
  Investee.create({
    ...
    .then((invest) => res.status(201).json(invest))
    .catch((err) => {
      res.status(500).json({ error: err });
    });
};
...
const investeeUpdate = (req, res) => {
  ...
  return Investee.findByIdAndUpdate({
    investid,
    $set: {
...
const investDelete = (req, res) => {
  ...
  return Investee.findByIdAndRemove(investid)
    .then(() => res.status(200)
      .json({ message: 'successfully deleted the investee' }))
    .catch((err) => {
      res.status(500).json({ error: err });
    });
};
